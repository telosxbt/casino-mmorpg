...
// Static sprite preview: front-idle frame (col 1, row 0) of the 3x4 sheet.
function avatarPreview(sheet: 'male' | 'female'): React.CSSProperties {
  const W = 130, H = 103; // matches the 410:326 frame aspect
  return {
    width: W,
    height: H,
        ...
        <div className="cz-row" style={{ gap: 16 }}>
          {(['MALE', 'FEMALE'] as const).map((g) => (
            <button
              ...
              onClick={() => setGender(g)}
              className="cz-felt"
              style={{ cursor: 'pointer', padding: 12, border: 0, outline: gender === g ? '3px solid var(--gold-1)' : 'none', flex: 1 }}
            >
